				...
				<view class="d-flex justify-content-between align-items-baseline px-3 py-3 border-bottom">
					<view>
						...
						<view class="hstack gap-1">
							<text class="text-secondary f12">点击进入详情，查看记录信息</text>
							<!-- <image @click="copyOrderNo(item.order_no)" style="width: 12px; height: 12px;" :src="icons.copy"></image> -->
						</view>
						<view class="mt-2">
							<text class="text-secondary f12">{{item.content_type_text}}</text> 
							<text class="f14 ms-3">￥{{item.amount}}</text>
						</view>
					</view>
					<view>
...
export default {
	data() {
		return {
			...
			tabs: [
				...
				{ id: 1, name: '成功', disabled: false },
				{ id: 2, name: '生成中', disabled: false },
	...
	methods: {
		...mapActions(activityStore, ['getActivityList']),
		fetch() {
			this.getActivityList(this.params).then(resp => {
				// 追加数据
		...
		onChangeTab(index) {
			this.current = index
			this.params.status = this.statusOptions[index]
			this.params.page = 1
			this.items = []
			this.status = 'more'
			this.fetch()
